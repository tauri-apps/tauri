// Host-side app code — runs in a Worker with a live event loop. Mirrors the
// commands and events of examples/api/src-tauri/src (cmd.rs + lib.rs), so the
// shared Svelte frontend drives Deno instead of Rust.
...
app.command('echo', (payload: unknown) => payload)
...
app.listen('js-event', (payload: unknown) => {
