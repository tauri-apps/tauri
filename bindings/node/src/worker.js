...
export class WebviewWindow {
  ...
  innerSize() {
    ...
    const height = [0]
    check(
      api.webviewWindowInnerSize(this.#handle, width, height),
      'window.innerSize'
    )
  ...
  outerSize() {
    ...
    const height = [0]
    check(
      api.webviewWindowOuterSize(this.#handle, width, height),
      'window.outerSize'
    )
    return { width: width[0], height: height[0] }
  }
  innerPosition() {
    ...
    const y = [0]
    check(
      api.webviewWindowInnerPosition(this.#handle, x, y),
  ...
  outerPosition() {
    ...
    const y = [0]
    check(
      api.webviewWindowOuterPosition(this.#handle, x, y),
  ...
  availableMonitors() {
    const out = [null]
    check(
      api.webviewWindowAvailableMonitors(this.#handle, out),
  ...
  currentMonitor() {
    const out = [null]
    check(
      api.webviewWindowCurrentMonitor(this.#handle, out),
  ...
  primaryMonitor() {
    const out = [null]
    check(
      api.webviewWindowPrimaryMonitor(this.#handle, out),
  ...
  monitorFromPoint(x, y) {
    const out = [null]
    check(
      api.webviewWindowMonitorFromPoint(this.#handle, x, y, out),
  ...
  cursorPosition() {
    ...
    const y = [0]
    check(
      api.webviewWindowCursorPosition(this.#handle, x, y),
  ...
  setSize({ width, height, physical = false }) {
    check(
      api.webviewWindowSetSize(this.#handle, width, height, physical),
      'window.setSize'
    )
  }
  setPosition({ x, y, physical = false }) {
    check(
      api.webviewWindowSetPosition(this.#handle, x, y, physical),
      'window.setPosition'
    )
  }
  setFullscreen(fullscreen) {
    check(
      api.webviewWindowSetFullscreen(this.#handle, fullscreen),
      'window.setFullscreen'
    )
  }
  setResizable(resizable) {
    check(
      api.webviewWindowSetResizable(this.#handle, resizable),
      'window.setResizable'
    )
  }
  setAlwaysOnTop(alwaysOnTop) {
    check(
      api.webviewWindowSetAlwaysOnTop(this.#handle, alwaysOnTop),
      'window.setAlwaysOnTop'
    )
  }
  setDecorations(decorations) {
    check(
      api.webviewWindowSetDecorations(this.#handle, decorations),
  ...
  setClosable(closable) {
    check(
      api.webviewWindowSetClosable(this.#handle, closable),
      'window.setClosable'
    )
  }
  setMaximizable(maximizable) {
    check(
      api.webviewWindowSetMaximizable(this.#handle, maximizable),
      'window.setMaximizable'
    )
  }
  setMinimizable(minimizable) {
    check(
      api.webviewWindowSetMinimizable(this.#handle, minimizable),
      'window.setMinimizable'
    )
  }
  setAlwaysOnBottom(alwaysOnBottom) {
    check(
      api.webviewWindowSetAlwaysOnBottom(this.#handle, alwaysOnBottom),
      'window.setAlwaysOnBottom'
    )
  }
  setContentProtected(protected_) {
    check(
      api.webviewWindowSetContentProtected(this.#handle, protected_),
      'window.setContentProtected'
    )
  }
  setSkipTaskbar(skip) {
    check(
      api.webviewWindowSetSkipTaskbar(this.#handle, skip),
  ...
  setVisibleOnAllWorkspaces(visible) {
    check(
      api.webviewWindowSetVisibleOnAllWorkspaces(this.#handle, visible),
      'window.setVisibleOnAllWorkspaces'
    )
  }
  setIgnoreCursorEvents(ignore) {
    check(
      api.webviewWindowSetIgnoreCursorEvents(this.#handle, ignore),
      'window.setIgnoreCursorEvents'
    )
  }
  setCursorVisible(visible) {
    check(
      api.webviewWindowSetCursorVisible(this.#handle, visible),
      'window.setCursorVisible'
    )
  }
  setCursorGrab(grab) {
    check(
      api.webviewWindowSetCursorGrab(this.#handle, grab),
      'window.setCursorGrab'
    )
  }
  setEnabled(enabled) {
    check(
      api.webviewWindowSetEnabled(this.#handle, enabled),
      'window.setEnabled'
    )
  }
  setFocusable(focusable) {
    check(
      api.webviewWindowSetFocusable(this.#handle, focusable),
      'window.setFocusable'
    )
  }
  setSimpleFullscreen(enable) {
    check(
      api.webviewWindowSetSimpleFullscreen(this.#handle, enable),
  ...
  setMinSize({ width = 0, height = 0, physical = false } = {}) {
    check(
      api.webviewWindowSetMinSize(this.#handle, width, height, physical),
      'window.setMinSize'
    )
  }
  setMaxSize({ width = 0, height = 0, physical = false } = {}) {
    check(
      api.webviewWindowSetMaxSize(this.#handle, width, height, physical),
      'window.setMaxSize'
    )
  }
  setCursorPosition({ x, y, physical = false }) {
    check(
      api.webviewWindowSetCursorPosition(this.#handle, x, y, physical),
  ...
  setTheme(theme) {
    check(
      api.webviewWindowSetTheme(this.#handle, theme ?? ''),
      'window.setTheme'
    )
  }
  ...
  setCursorIcon(icon) {
    check(
      api.webviewWindowSetCursorIcon(this.#handle, icon),
  ...
  requestUserAttention(kind) {
    check(
      api.webviewWindowRequestUserAttention(this.#handle, kind ?? ''),
  ...
  setProgressBar(state) {
    check(
      api.webviewWindowSetProgressBar(
        this.#handle,
  ...
  setSizeConstraints(constraints) {
    check(
      api.webviewWindowSetSizeConstraints(
        this.#handle,
  ...
  setBackgroundColor({ r, g, b, a = 255 }) {
    check(
      api.webviewWindowSetBackgroundColor(this.#handle, r, g, b, a),
  ...
  setBadgeCount(count) {
    check(
      api.webviewWindowSetBadgeCount(this.#handle, count ?? -1),
  ...
  setBadgeLabel(label) {
    check(
      api.webviewWindowSetBadgeLabel(this.#handle, label ?? ''),
  ...
  setTitleBarStyle(style) {
    check(
      api.webviewWindowSetTitleBarStyle(this.#handle, style),
  ...
  setOverlayIcon(rgba, width = 0, height = 0) {
    check(
      api.webviewWindowSetOverlayIcon(this.#handle, rgba, width, height),
  ...
  navigate(url) {
    check(
      api.webviewWindowNavigate(this.#handle, String(url)),
      'window.navigate'
    )
  }
  reload() {
  ...
  clearAllBrowsingData() {
    check(
      api.webviewWindowClearAllBrowsingData(this.#handle),
...
export class Tray {
  ...
  setIconAsTemplate(isTemplate) {
    check(
      api.traySetIconAsTemplate(this.#handle, isTemplate),
  ...
  setShowMenuOnLeftClick(enable) {
    check(
      api.traySetShowMenuOnLeftClick(this.#handle, enable),
...
export class Window {
  ...
  label() {
    return this.#str(api.windowLabel, 'window.label')
  }
  title() {
    return this.#str(api.windowTitle, 'window.title')
  }
  theme() {
    return this.#str(api.windowTheme, 'window.theme')
  }
  scaleFactor() {
    const o = [0]
    check(api.windowScaleFactor(this.#handle, o), 'window.scaleFactor')
    return o[0]
  }
  innerSize() {
    const w = [0],
      h = [0]
    check(api.windowInnerSize(this.#handle, w, h), 'window.innerSize')
    return { width: w[0], height: h[0] }
  }
  outerSize() {
    const w = [0],
      h = [0]
    check(api.windowOuterSize(this.#handle, w, h), 'window.outerSize')
    return { width: w[0], height: h[0] }
  }
  innerPosition() {
    const x = [0],
  ...
  cursorPosition() {
    const x = [0],
      y = [0]
    check(api.windowCursorPosition(this.#handle, x, y), 'window.cursorPosition')
    return { x: x[0], y: y[0] }
  }
  ...
  isDecorated() {
    return this.#bool(api.windowIsDecorated, 'window.isDecorated')
  }
  isClosable() {
    return this.#bool(api.windowIsClosable, 'window.isClosable')
  }
  isMaximizable() {
    return this.#bool(api.windowIsMaximizable, 'window.isMaximizable')
  }
  isMinimizable() {
    return this.#bool(api.windowIsMinimizable, 'window.isMinimizable')
  }
  isAlwaysOnTop() {
    return this.#bool(api.windowIsAlwaysOnTop, 'window.isAlwaysOnTop')
  }
  isEnabled() {
    return this.#bool(api.windowIsEnabled, 'window.isEnabled')
  }
  isMenuVisible() {
    return this.#bool(api.windowIsMenuVisible, 'window.isMenuVisible')
  }

  availableMonitors() {
    return JSON.parse(
      this.#str(api.windowAvailableMonitors, 'window.availableMonitors') || '[]'
    )
  }
  currentMonitor() {
    return JSON.parse(
      this.#str(api.windowCurrentMonitor, 'window.currentMonitor') || 'null'
    )
  }
  primaryMonitor() {
    return JSON.parse(
      this.#str(api.windowPrimaryMonitor, 'window.primaryMonitor') || 'null'
    )
  }
  monitorFromPoint(x, y) {
    const o = [null]
    check(
      api.windowMonitorFromPoint(this.#handle, x, y, o),
      'window.monitorFromPoint'
    )
    return JSON.parse(takeString(o) || 'null')
  }

  setTitle(title) {
    this.#set(api.windowSetTitle, 'window.setTitle', title)
  }
  setSize({ width, height, physical = false }) {
    check(
      api.windowSetSize(this.#handle, width, height, physical),
      'window.setSize'
    )
  }
  setPosition({ x, y, physical = false }) {
    check(
      api.windowSetPosition(this.#handle, x, y, physical),
      'window.setPosition'
    )
  }
  setMinSize({ width = 0, height = 0, physical = false } = {}) {
    check(
      api.windowSetMinSize(this.#handle, width, height, physical),
      'window.setMinSize'
    )
  }
  setMaxSize({ width = 0, height = 0, physical = false } = {}) {
    check(
      api.windowSetMaxSize(this.#handle, width, height, physical),
      'window.setMaxSize'
    )
  }
  setCursorPosition({ x, y, physical = false }) {
    check(
      api.windowSetCursorPosition(this.#handle, x, y, physical),
      'window.setCursorPosition'
    )
  }
  setFullscreen(v) {
    this.#set(api.windowSetFullscreen, 'window.setFullscreen', v)
  }
  setResizable(v) {
    this.#set(api.windowSetResizable, 'window.setResizable', v)
  }
  setAlwaysOnTop(v) {
    this.#set(api.windowSetAlwaysOnTop, 'window.setAlwaysOnTop', v)
  }
  setAlwaysOnBottom(v) {
    this.#set(api.windowSetAlwaysOnBottom, 'window.setAlwaysOnBottom', v)
  }
  setDecorations(v) {
    this.#set(api.windowSetDecorations, 'window.setDecorations', v)
  }
  setClosable(v) {
    this.#set(api.windowSetClosable, 'window.setClosable', v)
  }
  setMaximizable(v) {
    this.#set(api.windowSetMaximizable, 'window.setMaximizable', v)
  }
  setMinimizable(v) {
    this.#set(api.windowSetMinimizable, 'window.setMinimizable', v)
  }
  setContentProtected(v) {
    this.#set(api.windowSetContentProtected, 'window.setContentProtected', v)
  }
  setSkipTaskbar(v) {
    this.#set(api.windowSetSkipTaskbar, 'window.setSkipTaskbar', v)
  }
  setShadow(v) {
    this.#set(api.windowSetShadow, 'window.setShadow', v)
  }
  setVisibleOnAllWorkspaces(v) {
    this.#set(
      api.windowSetVisibleOnAllWorkspaces,
      'window.setVisibleOnAllWorkspaces',
      v
    )
  }
  setIgnoreCursorEvents(v) {
    this.#set(
      api.windowSetIgnoreCursorEvents,
      'window.setIgnoreCursorEvents',
      v
    )
  }
  setCursorVisible(v) {
    this.#set(api.windowSetCursorVisible, 'window.setCursorVisible', v)
  }
  setCursorGrab(v) {
    this.#set(api.windowSetCursorGrab, 'window.setCursorGrab', v)
  }
  setEnabled(v) {
    this.#set(api.windowSetEnabled, 'window.setEnabled', v)
  }
  setFocusable(v) {
    this.#set(api.windowSetFocusable, 'window.setFocusable', v)
  }
  setSimpleFullscreen(v) {
    this.#set(api.windowSetSimpleFullscreen, 'window.setSimpleFullscreen', v)
  }
  setTheme(theme) {
    this.#set(api.windowSetTheme, 'window.setTheme', theme ?? '')
  }
  setCursorIcon(icon) {
    this.#set(api.windowSetCursorIcon, 'window.setCursorIcon', icon)
  }
  requestUserAttention(kind) {
    this.#set(
      api.windowRequestUserAttention,
      'window.requestUserAttention',
      kind ?? ''
    )
  }
  ...
  setBackgroundColor({ r, g, b, a = 255 }) {
    check(
      api.windowSetBackgroundColor(this.#handle, r, g, b, a),
      'window.setBackgroundColor'
    )
  }
  setBadgeCount(count) {
    this.#set(api.windowSetBadgeCount, 'window.setBadgeCount', count ?? -1)
  }
  setBadgeLabel(label) {
    this.#set(api.windowSetBadgeLabel, 'window.setBadgeLabel', label ?? '')
  }
  /** macOS only: 'visible', 'transparent' or 'overlay'. */
  setTitleBarStyle(style) {
    this.#set(api.windowSetTitleBarStyle, 'window.setTitleBarStyle', style)
  }
  /** Windows only: RGBA pixels (width*height*4 bytes), or null to clear. */
  setOverlayIcon(rgba, width = 0, height = 0) {
    check(
      api.windowSetOverlayIcon(this.#handle, rgba, width, height),
      'window.setOverlayIcon'
    )
  }
  /** macOS only: the NSWindow pointer, as an integer. */
  nsWindow() {
    const o = [0]
    check(api.windowNsWindow(this.#handle, o), 'window.nsWindow')
    return o[0]
  }
  /** macOS only: the NSView pointer, as an integer. */
  nsView() {
    const o = [0]
    check(api.windowNsView(this.#handle, o), 'window.nsView')
    return o[0]
  }
  /** Windows only: the window's HWND, as an integer. */
  hwnd() {
    const o = [0]
    check(api.windowHwnd(this.#handle, o), 'window.hwnd')
    return o[0]
  }
  setFocus() {
    this.#act(api.windowSetFocus, 'window.setFocus')
  }
  show() {
    this.#act(api.windowShow, 'window.show')
  }
  hide() {
    this.#act(api.windowHide, 'window.hide')
  }
  center() {
    this.#act(api.windowCenter, 'window.center')
  }
  maximize() {
    this.#act(api.windowMaximize, 'window.maximize')
  }
  unmaximize() {
    this.#act(api.windowUnmaximize, 'window.unmaximize')
  }
  minimize() {
    this.#act(api.windowMinimize, 'window.minimize')
  }
  unminimize() {
    this.#act(api.windowUnminimize, 'window.unminimize')
  }
  close() {
    this.#act(api.windowClose, 'window.close')
  }
  destroy() {
    this.#act(api.windowDestroy, 'window.destroy')
  }
  startDragging() {
    this.#act(api.windowStartDragging, 'window.startDragging')
  }
  hideMenu() {
    this.#act(api.windowHideMenu, 'window.hideMenu')
  }
  showMenu() {
    this.#act(api.windowShowMenu, 'window.showMenu')
  }
  /** Attaches a Menu as this window's menu. */
  setMenu(menu) {
    check(api.menuSetAsWindowMenu(this.#handle, menu.handle), 'window.setMenu')
  }
  free() {
    check(api.handleClose(this.#handle), 'window.free')
  }
...
export class Webview {
  ...
  label() {
    const o = [null]
    check(api.webviewLabel(this.#handle, o), 'webview.label')
    return takeString(o)
  }
  url() {
    const o = [null]
    check(api.webviewUrl(this.#handle, o), 'webview.url')
    return takeString(o)
  }
  position() {
    const x = [0],
      y = [0]
    check(api.webviewPosition(this.#handle, x, y), 'webview.position')
    return { x: x[0], y: y[0] }
  }
  size() {
    const w = [0],
      h = [0]
    check(api.webviewSize(this.#handle, w, h), 'webview.size')
    return { width: w[0], height: h[0] }
  }
  /** The parent window. */
  window() {
    const o = [0]
    check(api.webviewGetWindow(this.#handle, o), 'webview.window')
    return new Window(o[0])
  }
  eval(js) {
    check(api.webviewEval(this.#handle, js), 'webview.eval')
  }
  navigate(url) {
    check(api.webviewNavigate(this.#handle, String(url)), 'webview.navigate')
  }
  reload() {
    check(api.webviewReload(this.#handle), 'webview.reload')
  }
  print() {
    check(api.webviewPrint(this.#handle), 'webview.print')
  }
  setZoom(scale) {
    check(api.webviewSetZoom(this.#handle, scale), 'webview.setZoom')
  }
  setAutoResize(v) {
    check(api.webviewSetAutoResize(this.#handle, v), 'webview.setAutoResize')
  }
  setSize({ width, height, physical = false }) {
    check(
      api.webviewSetSize(this.#handle, width, height, physical),
      'webview.setSize'
    )
  }
  setPosition({ x, y, physical = false }) {
    check(
      api.webviewSetPosition(this.#handle, x, y, physical),
      'webview.setPosition'
    )
  }
  setBackgroundColor({ r, g, b, a = 255 }) {
    check(
      api.webviewSetBackgroundColor(this.#handle, r, g, b, a),
      'webview.setBackgroundColor'
    )
  }
  setFocus() {
    check(api.webviewSetFocus(this.#handle), 'webview.setFocus')
  }
  show() {
    check(api.webviewShow(this.#handle), 'webview.show')
  }
  hide() {
    check(api.webviewHide(this.#handle), 'webview.hide')
  }
  close() {
    check(api.webviewClose(this.#handle), 'webview.close')
  }
  clearAllBrowsingData() {
    check(
      api.webviewClearAllBrowsingData(this.#handle),
      'webview.clearAllBrowsingData'
    )
  }
  reparent(window) {
    check(api.webviewReparent(this.#handle, window.handle), 'webview.reparent')
  }
  openDevtools() {
    check(api.webviewOpenDevtools(this.#handle), 'webview.openDevtools')
  }
  closeDevtools() {
    check(api.webviewCloseDevtools(this.#handle), 'webview.closeDevtools')
  }
  isDevtoolsOpen() {
    const o = [false]
    check(api.webviewIsDevtoolsOpen(this.#handle, o), 'webview.isDevtoolsOpen')
    return o[0]
  }
  free() {
    check(api.handleClose(this.#handle), 'webview.free')
  }
}
...
export class MenuItem {
  ...
  id() {
    const o = [null]
    check(api.menuItemId(this.#handle, o), 'menuItem.id')
    return takeString(o)
  }
  setText(text) {
    check(api.menuItemSetText(this.#handle, text), 'menuItem.setText')
  }
  setEnabled(v) {
    check(api.menuItemSetEnabled(this.#handle, v), 'menuItem.setEnabled')
  }
  setChecked(v) {
    check(api.menuItemSetChecked(this.#handle, v), 'menuItem.setChecked')
  }
  setAccelerator(accel) {
    check(
      api.menuItemSetAccelerator(this.#handle, accel ?? ''),
      'menuItem.setAccelerator'
    )
  }
  /** Appends a child (only valid on a submenu). */
  append(item) {
    check(api.submenuAppend(this.#handle, item.handle), 'submenu.append')
  }
  free() {
    check(api.handleClose(this.#handle), 'menuItem.free')
  }
}
...
export class Menu {
  ...
  append(item) {
    check(api.menuAppend(this.#handle, item.handle), 'menu.append')
  }
  free() {
    check(api.handleClose(this.#handle), 'menu.free')
  }
}
...
export const app = {
  ...
  emit(event, payload) {
    check(
      api.appEmit(appId, event, JSON.stringify(payload ?? null)),
      `emit(${event})`
    )
  },
  ...
  emitTo(label, event, payload) {
    check(
      api.appEmitTo(appId, label, event, JSON.stringify(payload ?? null)),
      `emitTo(${event})`
    )
  },
  ...
  createWindow(config) {
    const out = [0]
    check(
      api.webviewWindowCreate(appId, JSON.stringify(config), out),
      `createWindow(${config?.label})`
    )
  ...
  createBareWindow(config) {
    const out = [0]
    check(
      api.windowCreate(appId, JSON.stringify(config), out),
  ...
  menuItem({ id = '', text, enabled = true, accelerator = '' }) {
    const out = [0]
    check(
      api.menuItemNew(appId, id, text, enabled, accelerator, out),
      'menuItem'
    )
  ...
  checkMenuItem({
    ...
    const out = [0]
    check(
      api.menuCheckItemNew(appId, id, text, enabled, checked, accelerator, out),
      'checkMenuItem'
    )
    return new MenuItem(out[0])
  },
  ...
  predefinedMenuItem(kind, text = '') {
    const out = [0]
    check(
      api.menuPredefinedItemNew(appId, kind, text, out),
      'predefinedMenuItem'
    )
    return new MenuItem(out[0])
  },
...
async function handleInvoke(message) {
  ...
  try {
    ...
    check(
      api.invokeResolve(message.id, JSON.stringify(result ?? null)),
      'invoke_resolve'
    )
  } catch (error) {
    api.invokeReject(
  ...
  try {
    response = handler
      ...
      : {
          status: 404,
          headers: {},
          body: Buffer.from(`no handler for ${message.scheme}://`)
        }
  } catch (error) {
    console.error(
...
async function pump() {
  for (;;) {
    ...
    if (code !== CODES.OK)
      throw new Error(
        `event pump failed (${code}): ${api.lastErrorMessage() ?? ''}`
      )
