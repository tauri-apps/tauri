...
pub fn target_triple() -> crate::Result<String> {
  let arch = if cfg!(target_arch = "x86") {
    ...
    return Err(anyhow!("Unable to determine target-architecture",));
  };
  ...
  let os = if cfg!(target_os = "linux") {
    ...
    return Err(anyhow!("Unable to determine target-os"));
  };
  ...
  let os = if cfg!(target_os = "macos") || cfg!(target_os = "freebsd") {
    ...
    let env = if cfg!(target_env = "gnu") {
      ...
      return Err(anyhow!("Unable to determine target-environment"));
    };
