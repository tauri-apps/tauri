...
pub struct InlinedPlugin {
  ...
  allow_all_commands_by_default: bool
}
...
impl InlinedPlugin {
  ...
  /// Adds all of the allow permissions (`allow-$command`) to plugin's `default.toml`
  pub fn allow_all_commands_by_default(mut self, allow: bool) -> Self {
    self.allow_all_commands_by_default = allow;
    self
  }
}
  ...
  for (name, plugin) in inlined_plugins {
    ...
    let mut permission_files = if plugin.commands.is_empty() {
      ...
      tauri_utils::acl::build::autogenerate_command_permissions(
        ...
        "",
        false,
        plugin.allow_all_commands_by_default,
      );
