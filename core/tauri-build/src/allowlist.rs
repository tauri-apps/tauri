...
pub fn check(config: &Config, manifest: &mut Manifest) -> Result<()> {
  ...
  for (plugin, conf) in &config.plugins.0 {
    let expected = if let Value::Object(obj) = conf {
      if let Some(Value::Object(allowlist)) = obj.get("allowlist") {
        let mut allowed_apis = Vec::new();
        for (allowed, value) in allowlist {
          if let Value::Bool(true) = value {
            allowed_apis.push(format!("allow-{}", AsKebabCase(allowed)));
          }
        }
        allowed_apis
      } else {
    ...
    dependencies.push(AllowlistedDependency {
      name: plugin.into(),
