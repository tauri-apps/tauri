...
impl Builder {
  ...
  pub fn try_build(self) -> Result<()> {
    ...
    if let Ok(commands) = std::fs::read_to_string(out_dir.join("commands.json"))
      .map_err(|_| ())
      .and_then(|c| serde_json::from_str::<Vec<String>>(&c).map_err(|_| ()))
    {
      acl::build::autogenerate_command_permissions(&commands_dir, &commands, "", true);
    }
