...
pub fn autogenerate_command_permissions(
  ...
  let mut slugified_commands = vec![];
  for command in commands {
    let slugified_command = command.replace('_', "-");
    slugified_commands.push(slugified_command.clone());
    let toml = format!(
...
"###,
    ...
    try_writing_file(toml, out_path)
  }

  let all_permissions = slugified_commands.iter()
      .map(|command| format!("\"allow-{}\"", command))
      .collect::<Vec<String>>()
      .join(",");

  let default_toml = if allow_all_by_default {
    format!(
      r###"{license_header}# Automatically generated - DO NOT EDIT!
[default]
...
"###,
  ...
  let out_path = path.join("default.toml".to_string());
  try_writing_file(default_toml, out_path)
}

fn try_writing_file(content: String, path: PathBuf) {
  if content != read_to_string(&path).unwrap_or_default() {
    std::fs::write(&path, content)
        .unwrap_or_else(|_| panic!("unable to autogenerate {path:?}"));
  }
}