...
fn app_manifest_permissions(
  ...
  let _ = fs::remove_dir_all(Path::new("permissions").join("autogenerated"));

  let mut permission_files = if manifest.commands.is_empty() {
    Vec::new()
  } else {
    // the autogenerated command permissions are written to `OUT_DIR` so they don't have to be
    // committed at the crate level.
    let autogenerated_path = app_out_dir.join("autogenerated");
    ...
    tauri_utils::acl::build::collect_permission_files(
  ...
  if let Some(pattern) = manifest.permissions_path_pattern {
    permission_files.extend(tauri_utils::acl::build::collect_permission_files(
      pattern,
      |_| true,
    )?);
  } else {
  ...
  let permission_files = tauri_utils::acl::build::define_permissions_from_files(
    permission_files,
    pkg_name,
    &app_out_dir,
  )?;
