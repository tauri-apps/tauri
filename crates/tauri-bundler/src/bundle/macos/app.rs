...
use crate::{
  Error::GenericError,
  Settings,
  bundle::settings::{PlistKind, WebviewRuntime},
...
pub fn bundle_project(settings: &Settings) -> crate::Result<Vec<PathBuf>> {
  ...
  if let WebviewRuntime::Cef {
    distribution,
    helper: _,
  } = settings.webview_runtime()
  {
    ...
    if let Some(cef_path) = distribution {
...
/// Creates the CEF helper .app bundles, building their executable for the
/// bundle's architecture (see [`cef_helper`]).
...
fn create_cef_helpers(bundle_directory: &Path, settings: &Settings) -> crate::Result<Vec<PathBuf>> {
  ...
  for helper_name in helpers {
    ...
    fs_utils::copy_file(&helper_binary, &helper_exec)?;
