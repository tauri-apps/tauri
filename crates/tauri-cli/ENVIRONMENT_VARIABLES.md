...
- `TAURI_SIGNING_PRIVATE_KEY` — Private key used to sign your app bundles. Can be either the key content as a string, or a path to a file containing the key (the path will be used if the file exists). Takes precedence over `TAURI_SIGNING_PRIVATE_KEY_PATH` unless the file doesn't exist.
- `TAURI_SIGNING_PRIVATE_KEY_PATH` — Path to the private key file used to sign your app bundles. If set, takes priority over `TAURI_SIGNING_PRIVATE_KEY` when the file exists.
