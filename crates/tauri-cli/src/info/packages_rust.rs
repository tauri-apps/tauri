...
pub async fn items(frontend_dir: Option<&PathBuf>, tauri_dir: Option<&Path>) -> Vec<SectionItem> {
  ...
  if tauri_dir.is_some() || frontend_dir.is_some() {
    if let Some(tauri_dir) = tauri_dir {
      ...
      for dep in ["tauri", "tauri-build", "wry", "tao"] {
        ...
        let item = rust_section_item(dep, crate_version).await;
        items.push(item);
      }
    }
  }

  let tauri_cli_rust_result = match tokio::process::Command::new("cargo")
    .arg("tauri")
    .arg("-V")
    .output()
    .await
    .ok()
  {
    Some(o) if o.status.success() => {
      ...
      let latest_version = crate_latest_version(package).await.unwrap_or_default();
      ActionResult::Description(format!(
        "{package} 🦀: {version}{}",
  ...
  items.push(SectionItem::new().action_result(tauri_cli_rust_result));

  items
}

pub async fn rust_section_item(dep: &str, crate_version: CrateVersion) -> SectionItem {
  let version = crate_version
    .version
    .as_ref()
    .and_then(|v| semver::Version::parse(v).ok());

  let version_suffix = match (version, crate_latest_version(dep).await) {
