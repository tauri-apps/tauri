...
pub async fn command() -> Result<()> {
  ...
  let manifest_contents = read_to_string(dirs.tauri.join("Cargo.toml")).await.fs_context(
  ...
  let workspace_dir = {
    ...
    tokio::task::spawn_blocking(move || get_workspace_dir(tauri_dir))
      .await
      .context("failed to wait on cargo metadata task")??
  };
  let lock_path = workspace_dir.join("Cargo.lock");
  let lock = if lock_path.exists() {
    let lockfile_contents = read_to_string(&lock_path)
      .await
      .fs_context("failed to read Cargo lockfile", &lock_path)?;
  ...
  if tauri_version.major == 1 {
    migrations::v1::run(&dirs)
      .await
      .context("failed to migrate from v1")?;
  } else if tauri_version.major == 2 {
    if let Some((pre, _number)) = tauri_version.pre.as_str().split_once('.') {
      match pre {
        "beta" => {
          migrations::v2_beta::run(&dirs)
            .await
            .context("failed to migrate from v2 beta")?;
        }
        "alpha" => {
          bail!(
