  ...
  if let Some((target_device_serial_no, target_device_name)) = target_device {
    ...
    for device in &devices {
      let reverse_list_output =
        hdc_reverse_list(env, device.id()).map_err(|error| crate::Error::CommandFailed {
          command: "hdc fport".to_string(),
          error,
        })?;
    ...
    if already_forwarded {
      ...
      loop {
        ...
        let reverse_list_output =
          hdc_reverse_list(env, &target_device_serial_no).map_err(|error| {
            crate::Error::CommandFailed {
              command: format!("hdc reverse {target_device_serial_no}"),
              error,
            }
          })?;
