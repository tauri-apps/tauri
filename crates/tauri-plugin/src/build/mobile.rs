...
/// Patches the entitlements file of the iOS project of the application that depends on this
/// plugin, so the plugin can request the capabilities its native code needs
/// (push notifications, app groups, HealthKit, iCloud...).
///
/// It edits `<project>/<app-name>_iOS/<app-name>_iOS.entitlements` of the Xcode project
/// generated by `tauri ios init`, reading its path from the `TAURI_IOS_PROJECT_PATH` and
/// `TAURI_IOS_APP_NAME` environment variables set by the Tauri CLI. It is a no-op when they are
/// not set (a plain `cargo build`, or a desktop build) or when the file does not exist, and the
/// file is only written when the closure actually changed it.
///
/// Must be called from the plugin `build.rs`, ideally guarded by a
/// [`plugin_config`](super::plugin_config) value so the app can opt in to the entitlement.
/// Only available on macOS hosts, where the iOS project can be built.
///
/// # Examples
///
/// ```rust,no_run
...
/// Patches the `Info.plist` of the iOS project of the application that depends on this plugin,
/// which is where iOS expects the usage descriptions, URL schemes and background modes a
/// plugin needs to be declared.
///
/// It edits `<project>/<app-name>_iOS/Info.plist` of the Xcode project generated by
/// `tauri ios init`, reading its path from the `TAURI_IOS_PROJECT_PATH` and
/// `TAURI_IOS_APP_NAME` environment variables set by the Tauri CLI. It is a no-op when they are
/// not set (a plain `cargo build`, or a desktop build) or when the file does not exist, and the
/// file is only written when the closure actually changed it.
///
/// Must be called from the plugin `build.rs`. Values that are specific to the application, such
/// as the text shown on a permission prompt, should be read from the plugin configuration with
/// [`plugin_config`](super::plugin_config) instead of hardcoded.
/// Only available on macOS hosts, where the iOS project can be built.
///
/// # Examples
///
...
///     "This app needs the camera to scan QR codes".into(),
///   );
/// })
