...
- [`364a0e711`](https://www.github.com/tauri-apps/tauri/commit/364a0e711d8c1e5f2ad9439feff498055f5065c7) ([#15882](https://www.github.com/tauri-apps/tauri/pull/15882)) `tauri-build` no longer copies the configured resources to the cargo target directory; on desktop, unbundled apps (`tauri dev` / `cargo run`) now resolve resources at runtime from their source paths instead. This means editing a resource file no longer triggers a full application rebuild, and changes to plain relative resources are picked up live by the running app.
...
- [`65b3b040a`](https://www.github.com/tauri-apps/tauri/commit/65b3b040a7248422e7b0820074c5fc70229216a7) The `tauri` custom protocol now resolves the asset path from the request URI path instead of stripping a hardcoded `tauri://localhost` prefix, so a runtime is free to define any custom scheme URL format in `Runtime::custom_scheme_url`.

### Performance Improvements

- [`fb7bdd95b`](https://www.github.com/tauri-apps/tauri/commit/fb7bdd95be319d70a1097d285bb7b170562339f2) Reduce the size of the resolved ACL embedded in the app by storing the autogenerated command permissions as a `commands` list on the plugin/app manifest instead of two explicit permissions (`allow-$command` and `deny-$command`) per command. The `allow-$command`/`deny-$command` permissions are now materialized on demand when resolving the ACL.
...
- [`c8c75b1f7`](https://www.github.com/tauri-apps/tauri/commit/c8c75b1f7f43e7cb1e7d773ed2f6f96fad2fe975) ([#15787](https://www.github.com/tauri-apps/tauri/pull/15787)) The Linux GTK bindings are now selected by the new `gtk3` and `gtk4` features instead of being always on, so `tauri` can be built for either GTK version and compiles on Linux and BSD without any GTK dependency at all.
...
- [`e3e3474d3`](https://www.github.com/tauri-apps/tauri/commit/e3e3474d33527242e7f4800f43348d0a8a95da38) On Android, `Invoke.resolveObject` and the other values serialized with the plugin JSON mapper now write `JSObject` and `JSArray` as JSON objects and arrays. They were serialized through their private fields, so a `JSArray` reached the frontend as `{ "values": [...] }` and a `JSObject` as `{ "nameValuePairs": {...} }`.
...
- [`97f6666b9`](https://www.github.com/tauri-apps/tauri/commit/97f6666b9b18411ae5d409feeb85cc9674c38487) ([#16088](https://www.github.com/tauri-apps/tauri/pull/16088)) Bump `tray-icon` to `0.25` and `muda` to `0.20` to pick up the fix for left-click events being swallowed by the tray menu on macOS 27 (fix #16035).
