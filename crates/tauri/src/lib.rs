...
//! - **macos-private-api-fullscreen**: Enables only the **macOS** `fullScreenEnabled` preference private API, without the transparent window functionality. Enabled by default if the `tauri > macOS > fullscreenApi` config flag is set to `true` on the `tauri.conf.json` file.
//! - **macos-private-api-transparent**: Enables only the **macOS** `transparent` window private API, without the `fullScreenEnabled` preference. Enabled by default if the `tauri > macOS > transparentBackgroundApi` config flag is set to `true` on the `tauri.conf.json` file.
