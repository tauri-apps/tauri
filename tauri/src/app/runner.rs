...
pub(crate) fn run<W: Webview + 'static>(application: App<W>) -> crate::Result<()> {
  ...
  let splashscreen_content = application
    .splashscreen_html()
    .map(|html| Content::Html(html.to_string()));
...
fn build_webview<W: Webview + 'static>(
  ...
  let hid_splashscreen = Arc::new(AtomicBool::new(false));
  ...
  webview.bind("__TAURI_INVOKE_HANDLER__", move |_, arg| {
    ...
    let hid_splashscreen = hid_splashscreen.clone();
    ...
    crate::async_runtime::spawn(async move {
      ...
      if arg == r#"{"cmd":"__initialized"}"# {
        ...
        if !hid_splashscreen.load(Ordering::Relaxed) {
          w.eval(&format!(r#"window.location.href = "{}""#, content_url));
          hid_splashscreen.swap(true, Ordering::Relaxed);
        }
      } else {
