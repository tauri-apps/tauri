...
use crate::{
  bundle::{
    ...
    windows::util::{
      ...
      WEBVIEW2_OFFLINE_INSTALLER_X64_URL, WEBVIEW2_OFFLINE_INSTALLER_X86_URL,
    },
  },
  Settings,
};
...
fn build_nsis_app_installer(
  ...
  match webview2_install_mode {
    WebviewInstallMode::EmbedBootstrapper { silent: _ } => {
      ...
      let online_bootstrapper_path = tauri_tools_path.join(guid);
    ...
    WebviewInstallMode::OfflineInstaller { silent: _ } => {
      ...
      let offline_installer_path = tauri_tools_path.join(arch).join(guid);
