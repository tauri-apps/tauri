...
fn get_tauri_dir() -> PathBuf {
  ...
  lookup(&cwd, |path, file_type| if file_type.is_dir() {
    path.join("tauri.conf.json").exists() || path.join("tauri.conf.json5").exists() || path.join("Tauri.toml").exists()
  ...
  .expect("Couldn't recognize the current folder as a Tauri project. It must contain a `tauri.conf.json`, `tauri.conf.json5` or `Tauri.toml` file in any subfolder.")
}
