...
impl IconMenuItem {
    ...
    /// light/dark appearance. Setting `Some(name)` overrides any previously
    /// configured icon (including [`Self::set_icon`] and
    /// [`Self::set_native_icon`]); `None` removes the image.
    ///
    /// ## Platform-specific:
    ///
    /// - **Windows / Linux**: Unsupported (no-op).
    pub fn set_sf_symbol<S: Into<String>>(&self, _symbol: Option<S>) {
        #[cfg(target_os = "macos")]
        self.inner
            .borrow_mut()
            .set_sf_symbol(_symbol.map(Into::into))
    }
