...
pub struct MenuChild {
    ...
    // SF Symbol icon (macOS only). When set, it takes precedence over `icon` and
    // `native_icon` on the rendered NSMenuItem. The image is configured as a
    // template image so it adapts to light/dark mode.
...
impl MenuChild {
    ...
    ///
    /// SF Symbols are template images, so they automatically adapt to the
    /// system light/dark appearance. Setting this clears any previously set
    /// `icon` or `native_icon`. Passing `None` removes the image entirely.
    pub fn set_sf_symbol(&mut self, symbol: Option<String>) {
        self.sf_symbol = symbol.clone();
        self.icon = None;
        self.native_icon = None;
        let accessibility = self.text.clone();
        for ns_items in self.ns_menu_items.values() {
            for ns_item in ns_items {
                menuitem_set_sf_symbol(ns_item, symbol.as_deref(), &accessibility);
            }
        }
    }
}
...
impl MenuChild {
    pub fn create_ns_item_for_submenu(
        ...
        unsafe {
            ...
            if let Some(symbol) = self.sf_symbol.as_deref() {
                menuitem_set_sf_symbol(&ns_menu_item, Some(symbol), &self.text);
            }
        }
...
fn menuitem_set_sf_symbol(menuitem: &NSMenuItem, symbol: Option<&str>, accessibility: &str) {
    let Some(symbol) = symbol else {
        unsafe { menuitem.setImage(None) };
        return;
    };

    unsafe {
        let name = NSString::from_str(symbol);
        let description = NSString::from_str(accessibility);
        let nsimage =
